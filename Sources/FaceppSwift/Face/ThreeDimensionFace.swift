...
public struct ThreeDimensionFaceOption: RequestProtocol {
    ...
    public init(params: [String : Any]) {
        ...
        if let url = params["image_url3"] as? String {
            imageURL3 = URL(string: url)
        }
        if let url = params["image_file3"] as? String {
            imageFile3 = URL(fileURLWithPath: url)
        }
        if let value = params["image_base64_3"] as? String {
            imageBase643 = value
        }
        if let value = params["texture"] as? Int {
            needTexture = value == 1
        }
